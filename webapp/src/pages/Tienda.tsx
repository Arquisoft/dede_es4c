...
export const CarritoContext = React.createContext(listaPorDefecto);
		...
		<div id="listadoProducto">
			<main>
				...
				>
					<Button
						variant="contained"
						sx={{ color: "#fff", m: 1, background: "#596886" }}
						endIcon={<RestaurantIcon />}
						onClick={handleFilterComida}
					>
						Comida
					</Button>
					<Button
						variant="contained"
						sx={{ color: "#fff", m: 1, background: "#596886" }}
						endIcon={<LocalDrinkSharpIcon />}
						onClick={handleFilterBebida}
					>
						Bebida
					</Button>
					<Button
					...
					<Button
						...
						sx={{ color: "#fff", m: 1, background: "#596886" }}
						endIcon={<GrassIcon />}
						onClick={handleFilterVegetariano}
					>
						Vegetarianos
					</Button>
					<Button
						variant="contained"
						sx={{ color: "#fff", m: 1, background: "#596886" }}
						endIcon={<FlutterDashIcon />}
						onClick={handleFilterNoVegetariano}
					>
						No Vegetarianos
					</Button>
				</Menu>
			</main>
			{isTodos() && <Productos products={listaTodos}></Productos>}
			{isComida() && <Productos products={listaComida}></Productos>}
			{isBebida() && <Productos products={listaBebida}></Productos>}
			{isVeg() && <Productos products={listaVeg}></Productos>}
			{isNoVeg() && <Productos products={listaNoVeg}></Productos>}
			{isNombre() && <Productos filtro={nombre + ""}></Productos>}
		</div>
	);
}
